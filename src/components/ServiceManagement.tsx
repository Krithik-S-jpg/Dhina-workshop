  ...
  const handleAddNew = async () => {
    if (newService.name && newService.price) {
      ...
      if (error) {
        ...
        alert(`Failed to add service: ${error.message || JSON.stringify(error)}`);
      } else {
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center">
          <div className="bg-white rounded-xl shadow-lg p-6 w-full max-w-md">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <input type="number" placeholder="Stock" value={editingService.stock ?? 0} onChange={(e) => setEditingService({ ...editingService, stock: Number(e.target.value) })} className="w-full px-3 py-2 border border-gray-300 rounded-lg" />
            </div>
            <div className="flex space-x-2 mt-4">
